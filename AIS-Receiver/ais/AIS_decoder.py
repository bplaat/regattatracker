...
def compute_checksum(msg):
    ...
    chcksum = 0
    # for every char in the ais sentenses (comma included)
    for c in msg[start:end]:
...
def decod_data(data):
    ...
    def decod_24(data):
        ...
        if not(ais_data['partno']):
            ...
            ais_data['vendorname'] = decod_str(data[48:90])
        return ais_data
    ...
    # if it fails, like a type 16 message, execute following code
    except (KeyError):
        logger.info("Cannot decode message type "+str(type_nb))
        ais_data = {'type': type_nb}
    except:
        # raise will raise the last error (WrongKey) and crash the script
        raise
    return ais_data
...
def decod_ais(msg):
    ...
    # usefull only if multi-line sentences.
    if s_size != '1':
        global globPayload
        ...
        if s_size != s_count:                   # if this isn't the last messages
            ...
            globPayload = globPayload + payload
            return {'none': 'empty'}
        else:
...
def format_heading(heading):
    # with leading zeroes
    return 'N/A' if heading == 511 else "{:3.1f}°".format(heading).zfill(6)
